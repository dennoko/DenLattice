...
namespace Dennokoworks.DenLattice.Editor
{
    ...
    /// 元メッシュアセットには一切書き込まない。
    /// </summary>
    internal static class DenLatticeBaker
    {
        private const string Suffix = "_lattice";

        internal static void Bake(DenLattice component)
        {
            ...
            if (created.Count > 0)
            {
                ...
                {
                    Debug.Log($"[Dennoko Lattice] ベイクしました: {AssetDatabase.GetAssetPath(asset)}", asset);
                }
            ...
            {
                Debug.LogWarning($"[Dennoko Lattice] ベイクをスキップしました — {message}", component);
            }

            if (created.Count == 0 && skipped.Count == 0)
            {
                Debug.LogWarning("[Dennoko Lattice] ベイク対象がありません（編集データが空です）。", component);
            }
        }

        /// <summary>
        /// 元メッシュ名に _lattice を付与する。既に付いている場合は追加しない。
        /// </summary>
        ...
        /// <summary>
        /// シェイプキー名も _lattice の規則に揃える。同名が既にある場合は連番を付ける。
        /// </summary>
