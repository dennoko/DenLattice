...
namespace Dennokoworks.DenLattice.Editor
{
    ...
    /// </summary>
    [InitializeOnLoad]
    internal static class DenLatticeVersion
    {
        ...
        private const string RepoName = "DenLattice";
        ...
        private const string VerCheckDoneKey = "DenLattice_VerCheck_Done";
        private const string VerCheckErrorKey = "DenLattice_VerCheck_Error";
        private const string VerCheckLatestKey = "DenLattice_VerCheck_Latest";
        private const string VerCheckUrlKey = "DenLattice_VerCheck_Url";
        private const string VerCheckMessageKey = "DenLattice_VerCheck_Message";

        // 以下はエディタ再起動をまたいで保持する必要があるため EditorPrefs に置く。
        // SessionState だと再起動のたびにリセットされ、レート制限中でも撃ち続けてしまう。
        private const string VerCheckLastAttemptKey = "DenLattice_VerCheck_LastAttemptUtc";
        private const string VerCheckCachedLatestKey = "DenLattice_VerCheck_CachedLatest";
        private const string VerCheckCachedUrlKey = "DenLattice_VerCheck_CachedUrl";
        private const string VerCheckCachedMessageKey = "DenLattice_VerCheck_CachedMessage";
        ...
        private static string LoadLocalVersion()
        {
            ...
            foreach (var guid in AssetDatabase.FindAssets("version t:TextAsset"))
            {
                ...
                if (!File.Exists(Path.Combine(dir, "Editor", "Version", "DenLatticeVersion.cs")) &&
                    !File.Exists(Path.Combine(dir, "Editor", "DenLatticeVersion.cs")) &&
                    !File.Exists(Path.Combine(dir, "Runtime", "DenLattice.cs"))) continue;
        ...
        private static string TryReadVersion(string path)
        {
            ...
            {
                Debug.LogWarning($"[Dennoko Lattice] version.json の読み込みに失敗しました ({path}): {e.Message}");
            }
            return null;
        }
