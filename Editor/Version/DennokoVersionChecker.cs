...
namespace Dennokoworks.DenLattice.Editor
{
    ...
    ///
    /// owner / repo は呼び出し側（<see cref="DenLatticeVersion"/>）から渡す。
    ...
    internal static class DennokoVersionChecker
    {
            ...
            string filePath, string localVersion, Action<Result> onResult)
        {
            ...
            catch (Exception e)
            {
                ...
                Debug.LogWarning($"[Dennoko Lattice] version check: request build failed: {e.Message}");
                onResult(Error(localVersion));
                return;
            }
            ...
            op.completed += _ =>
            {
                ...
                {
                    Debug.LogWarning($"[Dennoko Lattice] version check: callback failed: {e.Message}");
        ...
        private static Result BuildResult(UnityWebRequest req, string localVersion)
        {
            ...
            if (hasError)
            {
                ...
                Debug.LogWarning($"[Dennoko Lattice] version check 取得失敗: url={url} httpCode={req.responseCode} error={req.error} body={body}");
            ...
            {
                Debug.LogWarning($"[Dennoko Lattice] version check 取得失敗: レスポンスが空。url={url} httpCode={req.responseCode}");
                return Error(localVersion);
            }
            ...
            {
                Debug.LogWarning($"[Dennoko Lattice] version check 取得失敗: JSON パース失敗: {e.Message} url={url}");
                return Error(localVersion);
            }

            if (info == null || string.IsNullOrEmpty(info.version))
            {
                Debug.LogWarning($"[Dennoko Lattice] version check 取得失敗: version フィールドが空。url={url}");
                return Error(localVersion);
            }
