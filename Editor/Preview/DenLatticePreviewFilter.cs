...
namespace Dennokoworks.DenLattice.Editor
{
    ...
    /// </summary>
    internal class DenLatticePreviewFilter : IRenderFilter
    {
        public ImmutableList<RenderGroup> GetTargetGroups(ComputeContext context)
        {
            ...
            var byRenderer = new Dictionary<Renderer, List<DenLattice>>();
            var order = new List<Renderer>();

            foreach (var component in context.GetComponentsByType<DenLattice>())
            {
            ...
            ComputeContext context)
        {
            ...
            var components = group.GetData<List<DenLattice>>() ?? new List<DenLattice>();

            ObserveNodeInputs(context, components, group.Renderers);

            var node = new DenLatticePreviewNode(proxyPairs, components);
        ...
        /// ノード 1 つ分の入力を監視する。
        ///
        /// <see cref="DenLatticePreviewNode.Refresh"/> がノードを再利用する場合は
        ...
        /// </summary>
        private static void ObserveShape(ComputeContext context, DenLattice component, bool isEditing)
        {
            if (isEditing)
            {
        ...
        private static int EditingShapeFingerprint(DenLattice component)
        {
            return ComputeShapeFingerprint(component, false);
        }

        private static int ComputeShapeFingerprint(DenLattice component, bool includeHasEdits)
        {
