...
{
    internal class DenLatticePreviewNode : IRenderFilterNode
    {
        /// <summary>
        /// プロキシ 1 つ分の状態。
        /// </summary>
        ...
        private readonly List<DenLattice> _components;

        public RenderAspects WhatChanged => RenderAspects.Mesh;

        internal DenLatticePreviewNode(IEnumerable<(Renderer, Renderer)> proxyPairs, List<DenLattice> components)
        {
        ...
        private bool TryRebuild(Entry entry, Renderer original)
        {
            ...
            catch (Exception e)
            {
                ...
                if (!entry.LoggedFailure)
                {
                    ...
                    Debug.LogError(
                        $"[Dennoko Lattice] {original.name} のプレビューを更新できませんでした。\n{e}",
                        original);
                }
        ...
        private bool Rebuild(Entry entry)
        {
            ...
            DenLatticePreviewFilter.GatherResult kind;
            MeshEdit single;
            using (PreviewMarkers.GatherEdits.Auto())
            {
                kind = DenLatticePreviewFilter.GatherEditsInto(
                    _components, entry.Original, vertices.Count, entry.Merged, true, out single);
            }

            if (kind == DenLatticePreviewFilter.GatherResult.Empty)
            {
                DestroyGenerated(entry);
                return true;
            }
            ...
            if (entry.Generated == null)
            {
                ...
                entry.Generated.name = entry.Source.name + " (Dennoko Lattice)";
            ...
            try
            {
                if (kind == DenLatticePreviewFilter.GatherResult.Single)
                {
            ...
            RenderAspects updatedAspects)
        {
            ...
            DenLatticePreviewFilter.ObserveNodeInputs(context, _components, _entries.Keys);
...
#if DEN_LATTICE_DEBUG
        private static readonly HashSet<DenLatticePreviewNode> AliveNodes = new HashSet<DenLatticePreviewNode>();

        /// <summary>
        /// プレビューの生成メッシュと、ビルドと同じ経路（<see cref="DenLatticePreviewFilter.GatherEdits"/> +
        ...
        /// </summary>
        [MenuItem("Tools/dennokoworks/Dennoko Lattice/Debug/Compare Preview With Build Result")]
        private static void CompareWithBuildResult()
        {
            ...
            foreach (var node in AliveNodes)
            {
                foreach (var entry in node._entries.Values)
                {
                    ...
                    var edit = DenLatticePreviewFilter.GatherEdits(
                    ...
                    try
                    {
                        ...
                        mismatched++;
                        Debug.LogWarning(
                            $"[Dennoko Lattice] {entry.Original.name}: プレビューとビルド結果の頂点が一致しません。",
                            entry.Original);
                    }
                    finally
                    {
            ...
            Debug.Log(
                $"[Dennoko Lattice] プレビューとビルド結果の比較 — 比較 {compared} / 不一致 {mismatched} / 対象外 {skipped}");
        }
