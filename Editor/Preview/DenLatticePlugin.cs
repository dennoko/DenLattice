...
using UnityEngine;
using Dennokoworks.DenLattice;

[assembly: ExportsPlugin(typeof(Dennokoworks.DenLattice.Editor.DenLatticePlugin))]
...
namespace Dennokoworks.DenLattice.Editor
{
    ...
    /// </summary>
    internal class DenLatticePlugin : Plugin<DenLatticePlugin>
    {
        public override string QualifiedName => "dennokoworks.den-lattice";
        public override string DisplayName => "Dennoko Lattice";
        ...
        protected override void Configure()
        {
            InPhase(BuildPhase.Transforming)
                ...
                .PreviewingWith(new DenLatticePreviewFilter());
        }

        private static void Execute(BuildContext context)
        {
            var components = context.AvatarRootObject.GetComponentsInChildren<DenLattice>(true);
            if (components.Length == 0) return;

            var componentList = new List<DenLattice>(components);
            ...
            foreach (var component in components)
            {
                ...
                foreach (var edit in component.edits)
                {
                    ...
                    {
                        Debug.LogWarning(
                            $"[Dennoko Lattice] {edit.target.name} はアバター配下にないため適用をスキップしました。"
        ...
        private static void ApplyToRenderer(BuildContext context, Renderer target, List<DenLattice> components)
        {
            ...
            var merged = DenLatticePreviewFilter.GatherEdits(
                components, target, sourceMesh.vertexCount, skipped);

            foreach (var message in skipped)
            {
                Debug.LogWarning($"[Dennoko Lattice] {target.name}: {message}", target);
            }
