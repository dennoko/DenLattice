...
| 🔍 要検証（内部 API リフレクション依存） | AtlasCraft, BoxWeightTransfer, DenEmo, DennokoPSDEditor, Uni-Shooting, Uni-Vader |
...
> **DenLattice と DennokoMeshEditor の選択アウトライン抑制は廃止済み。**
> 両ツールの該当する UnityEditor 内部 API 依存は解消している。
